...
uint8_t AT_SAPBR_2[]="AT+SAPBR=3,1,\"APN\",\"UNIWAP\"\r\n";//cmnent
uint8_t AT_SAPBR_3[]="AT+SAPBR=1,1\r\n";
uint8_t AT_SAPBR_4[]="AT+SAPBR=2,1\r\n";//分配IP地址
...
uint8_t send_command(uint8_t *command,uint8_t *wait_sty,uint8_t wait_con,uint8_t *Receive_data_flag,uint8_t *rec_buf)
{
...
//					  printf("wait_tim_count=%d",wait_tim_count);
					 if(strstr((const char*)rec_buf,(const char*)wait_sty))
					 {
						 
			     	if(strstr((const char*)rec_buf,(const char*)"CCLK:"))
						{
					printf("rec_buf=%s\r\n",rec_buf);
             timePtr = strstr((const char*)rec_buf,(const char*)"CCLK:");
...
void get_ntp_time(uint8_t * Receive_data_flag,uint8_t *rec_buf)
{
	
	   send_command(AT_CNTP,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	   send_command(AT_CNTP_1,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	   send_command(AT_CCLK,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	

}
void gsm_register(uint8_t * Receive_data_flag,uint8_t *rec_buf)
{           
	   //send_command("AT+RST\r\n","OK",5,Receive_data_flag,rec_buf);
	   send_command("AT\r\n",(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	   send_command(AT_HTTPTERM,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	   send_command(AT_SAPBR_1,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	   send_command(AT_SAPBR_2,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	   send_command(AT_SAPBR_3,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
		 send_command(AT_SAPBR_4,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
     get_ntp_time(Receive_data_flag,rec_buf);
	   send_command(AT_HTTPINIT,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);
	send_command(AT_HTTPPARA,(uint8_t *)"OK",100,Receive_data_flag,rec_buf);	
}
...
uint8_t gsm_post_get_time(uint8_t *Receive_data_flag,uint8_t *rec_buf)
{
	       ...
	       //first step 发送域名PATH
         if(send_command(gsm_param.weather_api_path,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
				 {
					 
	       //two step 发送文本类型   
	         if( send_command(gsm_param.gsm_context_type,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
					 {
	       //three step 发送的参数大小
	            sprintf (data_numeb,"AT+HTTPDATA=%d,10000\r\n",strlen((const char *)gsm_param.weather_api_param)); //
//	        printf("data_numeb=%s",data_numeb);
	             if( send_command((uint8_t *)data_numeb,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
	       //four step 发送的参数
	                   {
 											 if(send_command(gsm_param.weather_api_param,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
											 {//等待时间要长
	       //five step  采用POST方式发送
	                        if(send_command(gsm_param.post_get,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
													{
														state=1;
														return  state;
...
//  printf("len=%d\r\n",strlen((const char *)gsm_param.get_info_api_param));
	       ...
	       //first step 发送域名PATH
           if(send_command(gsm_param.get_info_api_path,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
					 {
	       //two step 发送文本类型   
	           if(send_command(gsm_param.gsm_context_type,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
						 {
...
//	        printf("data_numeb=%s",data_numeb);
	        //three step 发送的参数大小命令
	               if(send_command((uint8_t *)data_numeb,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
								 {
	            //four step 发送的参数
	                 if(send_command(gsm_param.get_info_api_param,(uint8_t *)"OK",255,Receive_data_flag,rec_buf))
									 {//等待时间要长
	       //five step  采用POST方式发送
	                      if(send_command(gsm_param.post_get,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
												{
...
uint8_t gsm_post_openpsuh(uint8_t *Receive_data_flag, uint8_t *rec_buf,
...
//	        printf("data_numeb=%s",data_numeb);
	       ...
	       //four step 发送的参数
	                ...
	                if( send_command(gsm_param.get_info_api_param,(uint8_t *)"OK",50,Receive_data_flag,rec_buf))//等待时间要长
									{
	       //five step  采用POST方式发送
	                     if(send_command(gsm_param.post_get,(uint8_t *)"OK",5,Receive_data_flag,rec_buf))
											 {
